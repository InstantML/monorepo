...
- The SDK should remain backend-agnostic at the REST-contract level. It targets the primary Rust/Postgres server by default and keeps compatibility with the deprecated Node server through the same documented routes. Use `RLOBS_API_KEY`/`api_key` when hosted or API-key auth is required.
