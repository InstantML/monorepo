...
receives `init`, `log`, and `finish`, while InstantML mirrors scalar metrics
best-effort. InstantML failures warn and never interrupt W&B logging.
...
Use `instantml_project=...`, `instantml_api_key=...`, or
`instantml_base_url=...` on `wandb.init(...)` when the InstantML mirror should
use different settings from W&B.
```
...
`instantml sync` replays an offline run directory (or an offline root containing several `offline/<run_id>/` directories) to the server. `sync tensorboard …` remains a named subcommand; any other first token is treated as a path. A directory literally named `tensorboard` must be passed as `./tensorboard` to disambiguate.
...
2. Promotes crash-left partial segments (dead writer PID) to visible segments, so a hard-killed run's fsynced tail is delivered too. If an active partial remains — the writer PID is still alive, or the run is being synced from a different host where the PID check is meaningless — sync delivers what is visible, refuses to mark the directory synced, and exits `3` with a warning. Pass `--assume-dead` to force-promote remaining partials; only safe when the training process is known dead, since a still-running producer could lose its buffered tail.
...
6. After a full drain, applies the finish status recorded in the directory (a hard-killed run with no finish signature leaves the server run as-is and reports the unclean shutdown), PUTs the session manifest under the run's original producer session — `state: "final"` on success or permanent failure (with failed counts), `state: "active"` when a retryable remainder or active partial remains, gracefully skipped with a notice if the server predates the session route — and writes a `synced` marker into `sync-state.json`. A subsequent sync of a synced directory is a pure local no-op with zero requests. Events the sync queue cannot stage (oversized beyond the 64 MiB sync cap, local disk refusal) are reported as permanent failures and never skipped over.
