...
- High-load dashboard render paths build page-scoped run ID maps instead of scanning visible rows for every selected run, Compare row, stop-control candidate, and chart series label. The Metrics catalog aggregates requested metric keys per run in one bounded pass without per-metric carrier arrays, keeping re-renders and filtered metric searches responsive as selections approach the 2,000-run cap.
