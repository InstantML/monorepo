...
- Objects workspace for cross-run rich logged objects: users can filter by kind, run search, key substring, and step range, then inspect bounded text, histogram, classification-eval, table, and media previews without fanning out through selected-run endpoints.
- Rollout gallery.
...
- Chart render hot paths reuse the main normalized series for unzoomed range
  views, build summary rows only when the summary table is opened, and use
  ...
  the monotonic-x hint to skip re-sorting normalized points. Normalization
  collects count plus x/y extents in one fused pass, emits fixed-shape
  normalized points (one hidden class for the canvas/hover loops), and builds
  polyline strings lazily on first read — dense-canvas charts stroke numeric
  points and never pay for path strings; sparse SVG charts, the hovered-series
  focus overlay, and SVG export materialize them on demand. Run
  `npm run benchmark:charts` and `npm run benchmark:charts:browser`; the
  2026-07-10 local M1 result (including both follow-up passes) is in
...
- Handlers that flow from the dashboard shell into memo()'d children (workspace panel cards, runs-table rows, rail rows, trace-tree nodes) are wrapped in a stable-identity hook (`ui/use-stable-handler.ts`), so shell re-renders (panel search keystrokes, selection toggles, polls) no longer re-render every panel chart and table row through fresh closure identities. Rail rows are a memo()'d component, so hover-highlight sweeps re-render only the two rows whose state changed. Insights explorers memoize scatter/parallel-coords geometry and reuse the default k-means clustering when the projection axes match, the Compare side-by-side memoizes its metric lookup/columns/matrix so sort clicks re-sort without rebuilding, evidence items build once per data change with search as a cheap filter pass, `formatNumber`/`formatMetricValue`/`formatCompact` reuse cached `Intl.NumberFormat` instances, and metric-key filtering skips re-sorting catalogs that are already normalized. Tab-owned derived data (metric catalog rows, alert/dataset rows, quick-search items) is gated on the visible tab / open palette, so background polls stop recomputing data for panes that aren't mounted.
...
- Browse active-run rich logged objects in Run Detail and Artifacts, or use `/dashboard/objects` for cross-run object search. The selected-run slice renders table previews, histogram bars, and media cards from `GET /api/runs/:id/objects` plus bounded `GET /api/objects/:id/rows` table reads; the cross-run explorer uses `GET /api/objects/explorer` and keeps previews bounded. Hidden tabs do not fetch object manifests, and Compare keeps using existing artifact context to avoid extra selected-run fan-out.
...
- `app/styles/objects.css` — cross-run logged object explorer
