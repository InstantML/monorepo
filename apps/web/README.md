...
- Settings now includes plan and usage visibility plus seat invite/list controls. Metric-point usage is shown for the current UTC calendar month with its reset date; Warehouse data, projects, runs, seats, artifacts, metric series, and API keys are retained-resource counts that do not reset monthly. Warehouse data prefers exact ClickHouse table bytes for dedicated tenant databases and falls back to metadata estimates only when exact per-org bytes are unavailable, such as shared-cell orgs. The topbar mirrors the highest usage percentage in a compact badge near account controls. The API tab now lists keys, creates copy-once keys, revokes keys, and still shows request snippets for SDK/API users.
