...
- Derived workspace tabs use current summaries, selected-run artifacts, local saved views, and documented API routes. They do not yet imply persistent alert, dataset, model registry, or integration credential storage; Reports is the exception and uses the persisted `/api/reports` surface.
