...
- Live follow is enabled by default for visible or selected `running` runs. The dashboard opens one capped same-origin `GET /api/live/runs` EventSource stream for up to 100 run IDs and 50 metric keys, debounces bounded dashboard refreshes when metric/log/status invalidations arrive, and keeps a five-second bounded watchdog poll so live state still advances if a browser or proxy holds an SSE connection without delivering events. Users can pause/resume live follow from the strip below the topbar.
- Run Detail treats `failed` and heartbeat-derived `crashed` runs as retryable states. V1 restart support is copyable Python snippets only: retry as a new linked run or resume the same UUID with `resume="must"`. One-click agent restart remains deferred until a launch-agent design exists.
