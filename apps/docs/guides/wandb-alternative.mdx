...
InstantML also publishes an [agent task benchmark](/guides/agent-mcp-benchmark) that measures how reliably an MCP-connected coding agent answers experiment questions through the InstantML MCP server compared with the W&B MCP server on identical data.
