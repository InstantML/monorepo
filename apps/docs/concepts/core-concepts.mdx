---
title: "Core concepts"
description: "Learn the workspace, project, run, metric, config, and artifact model behind the SDK, API, and dashboard."
---
...
<Info>
The API and billing surfaces call the same boundary an **organization** —
org-scoped API keys, org IDs. The dashboard and these docs say workspace; API
pages say organization where the API literally uses org.
</Info>
...
<Note>
Run names are not editable. After creation you can update a run's status,
tags, and notes, but not its name — pass a name at init time when the name
matters.
</Note>
...
[Pricing, limits, and billing](/guides/pricing-limits-billing) for the exact
numbers per plan.

## Next steps

- [Quickstart](/quickstart)
- [Log from Python](/sdk/logging)
