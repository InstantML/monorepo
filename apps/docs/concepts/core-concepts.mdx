...
project named **`uncategorized`** (same default as W&B, so dual-logging
pilots and migrations stay symmetric). You can rename or filter into it like
any other project.
...
sequence is the run's 1-indexed position in its project. Same shape and
intent as W&B (`fluffy-cloud-7`). Examples: `bold-falcon-1`,
`serene-glacier-42`, `radiant-river-127`. Adjective and noun are picked
deterministically from the org, project, and sequence, so concurrent
creates that race on the same sequence still get distinct names. You can
edit the name later from the Run Detail page or via the API.
...
Both are searchable in the dashboard and visible in comparison workflows.
