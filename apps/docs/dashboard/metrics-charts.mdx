...
max, best-at-step, and point count. Toggle a run's visibility to focus the chart
on a subset without changing the underlying selection. Pinned metrics render as
their own charts stacked under the primary one.
...
`im.Histogram.from_values(scores, bins=shared_edges)` under a stable object key.
...
note. CSV export contains the already plotted data,
