...
| Search | Match root name, trace id prefix, rollout id, or thread id. |

The workspace deep-links with `run_id`, `trace_id`, and `span_id` query
parameters, so copied URLs reopen the exact selected span.
...
the latest traces for the selected run, and links each row into the full Traces
workspace with the exact trace and root span selected.
