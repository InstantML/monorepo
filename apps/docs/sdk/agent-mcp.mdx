...
<Warning>
Keep API keys out of checked-in config files. Prefer environment variables for shared workspaces or team repositories.
</Warning>
...
3. Copy the setup snippet for Claude Code, Codex, Cursor, VS Code, or a local bridge from the **Connect Agent** panel.
4. Restart or reconnect the client, then ask it to list your latest InstantML runs.

This dashboard flow keeps key creation and agent configuration in one place. Use the manual snippets below when setting up an agent before opening the dashboard.

## Sign in through the browser (OAuth, preview)

OAuth-capable clients can connect by URL alone and sign in through the browser instead of pasting a key. In the **Connect Agent** panel, choose **Browser sign-in** and copy the tokenless snippet, for example:
...
<Note>
Browser sign-in is in preview. When a workspace has not enabled OAuth, use an API key instead.
</Note>
...
- [Create and manage API keys](/dashboard/settings-api-keys)
- [Build reports in the dashboard](/dashboard/reports)
- [Query runs, metrics, and objects from Python](/sdk/querying-data)
- [API authentication and scopes](/api/authentication)
