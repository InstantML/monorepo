...
Browser sign-in is scoped to the workspace selected in the dashboard when you
copy the snippet. If you belong to multiple workspaces, add one MCP server per
workspace rather than using one connection for all orgs.

Browser sign-in grants read-only access as the signed-in user. Use an API key
when an agent needs write scopes such as report editing or SDK ingestion.
