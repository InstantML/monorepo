...
<Note>
`Api.runs()` remains available as the raw compatibility helper. It returns the decoded `/api/runs/summary` payload as a dictionary and also accepts legacy `project_id` and `offset` parameters. Prefer the typed query helpers for new code.
</Note>
...
- [Export data and usage limits](/guides/export-usage-limits)
- [Import runs from other trackers](/guides/imports)
- [Metrics series API reference](/api/metrics-series)
