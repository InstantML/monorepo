...
- Hosting preference: Google Cloud Run or equivalent container hosting for Rust services, InstantML-owned ClickHouse on Google Cloud for the current hosted beta, Cloudflare R2 for object storage, and Clerk or an equivalent managed auth provider for organizations and identity. Premium BYOC uses customer-owned self-hosted GCP ClickHouse through direct database credentials; broader managed-provider automation remains future/Enterprise only when cost, security, and isolation tradeoffs are justified.
