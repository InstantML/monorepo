...
- ✅ 3.2 Pinned labeled sidebar (wave 1) · 3.3 demote rarely-daily tabs (More group: Run health, Datasets, Imports; API deliberately stays top-level under Admin — it is the key-management surface new users need during onboarding) · 3.5 selection tray (Compare/Export/Clear, renders only with a selection) · 3.6 number-key tab jumps + Cmd+K retained.
...
- ✅ 3.4 one project control: the breadcrumb project crumb is now a real button that reveals the filter bar and opens the project selector (`crumb-project` in `topbar.tsx`); the filter-bar dropdown remains the single full selector and duplicate brandbar text stays demoted.
...
- 4.3 ✅ R1 (RunsTable mounted behind Panels⇄Table toggle) · R2/R4 (wave 1) · R3 (single checkbox affordance in table view) · R5 (swatch tooltip via table/chart identity) · ✅ R6 cross-highlighting shipped: hovering a rail run isolates its series in every line panel, hovering a chart series or legend chip lights the rail row and pops the chip (one `highlightRunId` channel through `WorkspaceSectionView`) · R7 (cap labels unified on "N plotted / scope") · R9 (table empty-match state) · R10 · R11 (already at HEAD). 🟡 R3 the rail keeps both the header checkbox and the "select all matching" banner — they cover different scopes (page vs. query) and share one selection model; the table view has the single-checkbox affordance. 🟡 R8 pagination stays in the rail footer: the footer is sticky-visible alongside the rail, so moving it above the list would push run rows below the fold for no reach benefit.
- 4.4 ✅ D1–D6, D9 (preview-in-Files affordance + media preview), D10 (breadcrumb action) · 🟡 D7 the Data tab always fetches the inspected run with a run-specific empty message, but still charts the globally-selected metric; defaulting it to the run's own first logged metric is an open follow-up · 🟡 D8 system/* render readably (heartbeat as relative time, telemetry excluded from user tables); automatic GPU/CPU capture has an accepted SDK design in `2026-06-11-deferred-backend-workstreams.md` (opt-out sampler, `system/hardware/*` keys).
...
- 4.16 🟡 AP1 revoke confirm shipped; per-key "last used" metadata needs backend usage tracking (deferred — no design yet) · ✅ AP2 (wave 1) · ✅ AP3 key-management hint with `/docs/quickstart` + `/docs/api` links beside the create form.
...
Closed: runs table (top gap), the full chart polish set **including the y-axis log-scale toggle + manual y-range (P0-9)** — per-panel Log pill and "Y auto/min–max" popover on every `MetricChart` (log10 mapping shared by paths, gridlines, mini-range, and SVG export via `yMapper`/`logAxisTicks`; non-positive points dropped with the count disclosed; manual ranges clipped to the plot rect) — system-telemetry readability, Run health dedupe, the All-projects overview, and run↔chart cross-highlighting. Remaining matrix gaps: alert rules/channels and automatic hardware capture carry accepted designs in `2026-06-11-deferred-backend-workstreams.md`; sweeps, model registry, public projects, the mobile-first pass, and report comments are deferred by the accepted plan (`docs/2026-06-10-ui-ux-final-plan.md` §8) — each is a product workstream with backend surface area, not a frontend flow defect. Known follow-up from review: per-panel log/y-range state is chart-local and does not yet persist into panel settings (a fullscreened panel starts back at linear).

### Section 6 — security & performance
- 🟡 S1 the mandatory half shipped (`rank-insights/train.py` defaults to the local server like every other example); the audit's suggested SDK key-vs-host mismatch warning is not implemented (the new SDK warning is the PF5 finish-drain one) · S2 (JSON-LD escaping) · S3 (revoke confirm) · S4 (non-loopback local-auth warning) · S5 (X-Frame-Options added; full CSP already present — audit claim was stale).
