...
- Current product storage is ClickHouse operational records plus ClickHouse metric time series through `apps/rust-server`, with local artifact storage behind an abstraction and future S3-compatible object storage planned for hosted bytes.
